/**
 * Focus on browse/library patterns that seem to get responses
 */

import * as net from 'net';
import * as crypto from 'crypto';

const HOST = 'YOUR_CORE_IP';
const PORT = 9332;

const SERVER_BROKER_ID = Buffer.from('YOUR_SERVER_BROKER_ID', 'hex');
const MAGIC = Buffer.from('ROON');

const CONNECT_REQUEST_TEMPLATE = '';

function buildClientHello(clientBrokerId: Buffer): Buffer {
  return Buffer.concat([MAGIC, Buffer.from([0x01, 0x04]), SERVER_BROKER_ID, clientBrokerId]);
}

function buildProtocolRequest(): Buffer {
  return Buffer.concat([MAGIC, Buffer.from([0x01, 0x02])]);
}

function buildConnectRequest(clientBrokerId: Buffer): Buffer {
  const hex = CONNECT_REQUEST_TEMPLATE.replace('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX', clientBrokerId.toString('hex'));
  return Buffer.from(hex, 'hex');
}

function buildMethodCall(msgId: number, typeName: string): Buffer {
  const parts: Buffer[] = [];
  parts.push(Buffer.from([0x47, msgId & 0xff]));
  parts.push(Buffer.from([0x81, 0x67, 0x00, 0x00, 0x00, 0x01, 0x00, 0x01]));
  parts.push(Buffer.from([typeName.length]));
  parts.push(Buffer.from(typeName));
  parts.push(Buffer.from([0x05, 0x03, 0x05, 0x03]));
  return Buffer.concat(parts);
}

async function test(): Promise<void> {
  const clientBrokerId = crypto.randomBytes(16);

  return new Promise((resolve) => {
    const socket = new net.Socket();
    let step = 0;
    let msgId = 1;
    let responses: { method: string; response: string }[] = [];

    socket.setTimeout(90000);

    socket.on('connect', () => {
      console.log('Connected\n');
      step = 1;
      socket.write(buildClientHello(clientBrokerId));
    });

    socket.on('data', (data) => {
      if (data.length >= 6 && data.subarray(0, 4).toString() === 'ROON') {
        const code = data[5];
        if (step === 1 && code === 0x80) {
          step = 2;
          socket.write(buildProtocolRequest());
        }
        else if (step === 2 && code === 0x82) {
          step = 3;
          socket.write(buildConnectRequest(clientBrokerId));
        }
      }
      else if (step === 3 && data[0] === 0x80) {
        step = 4;
        console.log('Session ready\n');
        runTests();
      }
      else if (step === 4 && data[0] === 0xc0) {
        // Response to our test
        const typeMatch = data.toString('utf8').match(/Sooloos\.[A-Za-z.]+/g);
        const types = typeMatch || [];
        const text = data.toString('utf8').replace(/[^\x20-\x7E]/g, ' ').trim();

        responses.push({
          method: 'pending',
          response: types.join(', ') || text.substring(0, 50)
        });
      }
    });

    async function runTests(): Promise<void> {
      // Try many variations on the Library.BrowseAlbums pattern
      const methods = [
        // Variations on BrowseAlbums
        'Sooloos.Msg.Library.BrowseAlbumsRequest',
        'Sooloos.Msg.Library.BrowseTracksRequest',
        'Sooloos.Msg.Library.BrowseArtistsRequest',
        'Sooloos.Msg.Library.BrowseRequest',
        'Sooloos.Msg.Library.QueryRequest',

        // Try with "Api" namespace
        'Sooloos.Msg.Api.Library.BrowseRequest',
        'Sooloos.Msg.Api.BrowseRequest',

        // Try Broker.Api pattern (like documented types)
        'Sooloos.Msg.Broker.Api.Library.BrowseRequest',

        // Try simple variations
        'Sooloos.Msg.Browse.Request',
        'Sooloos.Msg.Query.Request',

        // Try getting data that might exist
        'Sooloos.Msg.Library.GetAlbumsRequest',
        'Sooloos.Msg.Library.GetTracksRequest',
        'Sooloos.Msg.Library.GetArtistsRequest',

        // Try subscription patterns
        'Sooloos.Msg.Library.SubscribeRequest',
        'Sooloos.Msg.Subscribe.Request',

        // Try info patterns
        'Sooloos.Msg.Library.GetInfoRequest',
        'Sooloos.Msg.GetInfoRequest',
        'Sooloos.Msg.InfoRequest',
      ];

      console.log('Testing Library patterns (50ms intervals):\n');

      for (const method of methods) {
        msgId++;
        console.log(`  Testing: ${method}`);
        socket.write(buildMethodCall(msgId, method));
        await new Promise(r => setTimeout(r, 50));
      }

      console.log('\n  Waiting 3s for responses...\n');
      await new Promise(r => setTimeout(r, 3000));

      console.log(`Got ${responses.length} responses:`);
      for (const r of responses) {
        console.log(`  ${r.response}`);
      }

      // Try keepalive
      console.log('\nSending keepalive...');
      socket.write(Buffer.from([0x41, 0xff, 0x00]));

      await new Promise(r => setTimeout(r, 1000));
      socket.end();
    }

    socket.on('close', () => {
      console.log('Done');
      resolve();
    });

    socket.on('error', console.error);
    socket.connect(PORT, HOST);
  });
}

test().catch(console.error);
